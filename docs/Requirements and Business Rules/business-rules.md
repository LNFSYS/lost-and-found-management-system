...
| BR-09 | Media uploads must conform to format/size/count limits; item images and private evidence images must be clearly separated. | UC-048, UC-049, UC-050, UC-051, UC-087 | Implemented for MVP |
| BR-10 | Claim evidence, private media, contact info, chat rooms, and notifications must only be visible to authorized users. Public post contact info is masked by default; final production privacy wording remains a policy hardening task. | UC-049, UC-054, UC-078, UC-079, UC-083 | Implemented for MVP |
...
| BR-20 | Overdue items must be processed through explicit orders or batches: disposal order, donation batch, transfer, or extension depending on campus policy. Disposal/donation/transfer is blocked while related claims or return appointments are pending or accepted. | UC-018, UC-019, UC-020 | Partial |
...
| BR-23 | Realtime chat opens only for users involved in a valid claim; text and image messages must be saved and broadcast to the correct room; seen/unread state must be reflected in the web UI. Reconnect/offline/socket isolation tests remain backlog. | UC-079, UC-080, UC-081, UC-082 | Implemented |
...
| BR-30 | Testing/hardening must cover claim transitions, warehouse lifecycle, matching thresholds, realtime room isolation, evidence privacy, release checks, and migration from a blank DB. | UC-007, UC-015, UC-054, UC-071, UC-078, UC-089 | Partial |
