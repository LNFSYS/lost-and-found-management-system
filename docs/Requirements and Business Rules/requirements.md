...
| NFR-PERF-01 | Board/search/matching/warehouse/notification queries must use indexes and pagination; text search uses MySQL FULLTEXT; matching uses a bounded SQL candidate prefilter, background queue, materialized results, and batch suggestion reads. | UC-046, UC-047, UC-068, UC-072, UC-084 | P0 | Implemented for MVP scale; load testing remains future work |
