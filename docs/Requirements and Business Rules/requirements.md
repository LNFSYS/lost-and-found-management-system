...
| FR-MATCH-01 | The system must enqueue matching after create/update/upload, compute tiered scores in a retryable worker, include image/OCR signals, save materialized results, return suggestions, explain reasons, and allow manual re-run. | UC-068, UC-069, UC-070, UC-071, UC-072, UC-074, UC-075, UC-076 | P0 | Implemented |
...
| NFR-PERF-01 | Board/search/matching/warehouse/notification queries must use indexes and pagination; text search uses MySQL FULLTEXT and matching is processed through a background queue with materialized results. | UC-046, UC-047, UC-068, UC-072, UC-084 | P0 | Implemented for MVP scale; load testing remains future work |
| NFR-RT-01 | Socket realtime must have auth, accepted-claim gating, room isolation, unread state, and must not leak events between claim rooms. Reconnect/device network testing remains hardening. | UC-077, UC-078, UC-079, UC-080, UC-082, UC-083 | P1 | Implemented for MVP; reconnect matrix pending |
