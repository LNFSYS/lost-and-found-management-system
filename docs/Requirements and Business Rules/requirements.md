...
| --- | --- | --- | --- | --- |
| FR-AUTH-01 | The system must support OTP registration, account creation, audience role selection, blocking re-registration of active emails, and user activation. | UC-031, UC-032 | P0 | Implemented |
...
| FR-CLAIM-03 | The system must verify uploaded claim evidence, calculate an ownership confidence percentage, and show that percentage to the finder/staff reviewer. | UC-089, UC-090, UC-092 | P1 | Implemented |
...
| FR-WAREHOUSE-01 | Staff/Admin must manage warehouse items, status changes, receive/store/return operations, storage logs, and retention deadlines. | UC-011, UC-012, UC-013, UC-014, UC-015, UC-059, UC-060, UC-061 | P0 | Implemented |
| FR-WAREHOUSE-02 | The system must handle overdue warehouse items, create disposal orders, create donation batches, and alert staff/admin. | UC-016, UC-017, UC-018, UC-019, UC-020 | P1 | Partial |
| FR-APPT-01 | The system must support creating, accepting, rejecting, rescheduling/canceling, and completing return appointments after valid claim decisions. | UC-021, UC-022, UC-023, UC-024 | P1 | Implemented |
...
| NFR-SEC-02 | APIs requiring auth must validate JWT; missing permissions must return clear 401/403 responses. | UC-001, UC-002, UC-054, UC-062, UC-078 | P0 | Partial |
| NFR-PRIV-01 | Claim evidence, contact info, private media, chat rooms, and notifications must only be returned to authorized users. | UC-049, UC-054, UC-079, UC-083, UC-089 | P0 | Partial |
...
| NFR-AI-01 | AI/matching/evidence scoring serves only as decision support; it must not automatically approve claims or item returns. | UC-070, UC-076, UC-089, UC-090, UC-092 | P0 | Partial |
| NFR-AUDIT-01 | Sensitive operations such as claim transitions, warehouse processing, moderation, configuration, and evidence review must have log/audit trails. | UC-003, UC-004, UC-005, UC-006, UC-015, UC-066, UC-085 | P1 | Partial |
