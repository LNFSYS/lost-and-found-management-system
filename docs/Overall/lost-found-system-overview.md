...
2. Gửi thông báo in-app đến người dùng liên quan nếu điểm khớp vượt ngưỡng notification cấu hình (mặc định 80%)
...
Hiện tại hệ thống đã có notification in-app lưu trong database cho match tốt, kèm trạng thái đã đọc/chưa đọc. Realtime Socket.IO và email fallback vẫn là phạm vi mở rộng.
...
**Quản lý vòng đời đồ thất lạc quá hạn lưu trữ:**

Hệ thống hỗ trợ quản lý vòng đời của đồ thất lạc sau khi được bàn giao vào kho. Mỗi món đồ có thời hạn lưu giữ theo quy định. Khi gần hết hạn, hệ thống tự động cảnh báo cho quản trị viên. Nếu quá hạn mà không có người nhận, quản trị viên có thể cập nhật trạng thái thành quyên góp, thanh lý, hủy hoặc chuyển giao cho bộ phận liên quan. Toàn bộ quá trình xử lý phải được ghi nhận để đảm bảo minh bạch và dễ kiểm tra.

**Thời hạn lưu giữ đề xuất:**

| Loại đồ | Thời hạn đề xuất | Hướng xử lý ưu tiên |
|---|---:|---|
| Đồ thường | 30-60 ngày | Lưu kho, cảnh báo trước hạn, xử lý nếu quá hạn |
| Đồ giá trị cao | 90 ngày | Lập biên bản, lưu thêm hoặc bàn giao bảo vệ/CTSV |
| Giấy tờ cá nhân/thẻ sinh viên | Theo quy định trường | Ưu tiên liên hệ hoặc chuyển CTSV/bảo vệ |

**Trạng thái vòng đời kho đề xuất:**

```
PENDING_APPROVAL → Chờ xác nhận nhập kho
STORED           → Đang lưu giữ trong kho
CLAIMED          → Đã có người claim hợp lệ
RETURNED         → Đã trả cho chủ sở hữu
EXPIRED          → Quá hạn lưu giữ
DISPOSED         → Đã hủy/thanh lý theo quy định
DONATED          → Đã quyên góp
TRANSFERRED      → Đã chuyển bộ phận liên quan
```

**Xử lý sau quá hạn:**
- Đồ giá trị thấp: thanh lý hoặc hủy theo quy định của trường
- Đồ còn dùng được: quyên góp cho bộ phận/cộng đồng phù hợp
- Giấy tờ cá nhân/thẻ sinh viên: chuyển CTSV, bảo vệ hoặc đơn vị được phân công
- Đồ giá trị cao: lập biên bản, lưu thêm hoặc bàn giao bảo vệ/CTSV

**Biên bản xử lý quá hạn cần có:**
- Lý do xử lý
- Người xử lý
- Ngày xử lý
...
- Mỗi kho/điểm lưu giữ có sức chứa tối đa, ví dụ Kho A chứa tối đa 200 món
- Khi đạt 80% sức chứa, hệ thống cảnh báo admin
- Khi đạt 100%, hệ thống không cho chọn kho đó để nhận thêm đồ hoặc đề xuất kho khác còn sức chứa
...
Quy trình xử lý ảnh:
1. Upload ảnh lên Cloudinary và lưu metadata gồm `secure_url`, `public_id`, `format`, `size`, `media_kind`
2. Phân biệt ảnh vật phẩm `ITEM` và ảnh bằng chứng/phụ trợ `EVIDENCE`
3. Chỉ gọi **Google Cloud Vision API** cho ảnh `ITEM` để nhận diện nội dung ảnh
4. Ánh xạ kết quả nhận diện sang các danh mục hệ thống (Điện tử, Giấy tờ, v.v.)
5. Tự động điền sẵn trường "Danh mục" — người dùng chỉ cần xác nhận hoặc điều chỉnh
6. Trích xuất các **tag** mô tả vật (ví dụ: "màu đen", "leather", "Apple logo") và gợi ý thêm vào mô tả
...
Ghi chú triển khai hiện tại: hệ thống đã gửi notification in-app ngay khi điểm matching đạt `matching.notification_threshold` 80%. Email tức thời, digest hằng ngày và phân tầng smart notification vẫn là phần mở rộng.
...
| UC-178 | Quản lý đồ thất lạc quá hạn lưu trữ | Admin/Staff | Cao |
| UC-179 | Cấu hình thời hạn lưu giữ theo loại đồ/mức độ giá trị | Admin | Trung bình |
| UC-180 | Cảnh báo admin khi đồ sắp quá hạn hoặc đã quá hạn | System | Cao |
| UC-181 | Lập biên bản xử lý đồ quá hạn | Admin/Staff | Cao |
| UC-182 | Quản lý sức chứa kho và ngưỡng cảnh báo | Admin | Trung bình |
| UC-183 | Chặn chọn kho đã đầy hoặc đề xuất kho khác | System | Trung bình |
...
| UC-178 | Quản lý đồ thất lạc quá hạn lưu trữ và cho phép admin chọn hướng xử lý cuối vòng đời. |
| UC-179 | Cấu hình thời hạn lưu giữ theo loại đồ/mức độ giá trị. |
| UC-180 | Tạo cảnh báo cho admin khi item còn 7 ngày tới hạn hoặc đã quá hạn. |
| UC-181 | Lưu biên bản xử lý quá hạn gồm lý do, người xử lý, ngày xử lý, ghi chú và ảnh minh chứng nếu có. |
| UC-182 | Quản lý sức chứa kho, tính phần trăm sử dụng và ngưỡng cảnh báo. |
| UC-183 | Chặn chọn kho đã đầy hoặc đề xuất kho khác còn sức chứa. |
...
| UC-165 | Cấu hình khu vực lớn và địa điểm cụ thể trong campus; vị trí chi tiết do user nhập khi đăng tin. |
...
| UC-178 | Trang Admin quản lý đồ quá hạn, lọc theo loại đồ/trạng thái/kho. |
| UC-181 | Form lập biên bản xử lý quá hạn, upload ảnh minh chứng nếu cần. |
| UC-182 | Hiển thị sức chứa kho, phần trăm sử dụng và cảnh báo 80%/100%. |
| UC-183 | UI vô hiệu hóa kho đã đầy và gợi ý kho khác còn sức chứa. |
